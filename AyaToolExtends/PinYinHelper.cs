...
namespace AyaGameEngine2D
{
    ...
    public class PinYinHelper
    {
        ...
        /// 定义拼音区编码数组
        /// </summary>
        private int[] _pinYinCodeValue = new int[]
            {
        ...
        /// 定义拼音数组
        /// </summary>
        private string[] _pinYinName = new string[]
            {
        ...
        /// 拼音表
        /// </summary>
        private string _ls_second_eng = "" +
        ...
        /// 汉字表
        /// </summary>
        private string _ls_second_ch = "亍丌兀丐廿卅丕亘丞鬲孬噩丨禺丿匕乇夭爻卮氐囟胤馗毓睾鼗丶亟" +
                    ...
                    else
                    {
                        switch (asc)
                        {
                            ...
                            case -5659:
                                pystr += "Tian"; break;
                            default:
                                for (int i = (this._pinYinCodeValue.Length - 1); i >= 0; i--)
                                {
                                    if (this._pinYinCodeValue[i] <= asc) //判断汉字的拼音区编码是否在指定范围内
                                    {
                                        pystr += this._pinYinName[i];//如果不超出范围则获取对应的拼音
                                        break;
                                    }
                                }
                                break;
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                else if (array[0] >= 176 && array[0] <= 215)
                {
                    if (str[i].ToString().CompareTo("匝") >= 0)
                        ...
                        return_py += "x";
                    else if (str[i].ToString().CompareTo("哇") >= 0)
                        return_py += "w";
                ...
                {
                    return_py += this._ls_second_eng.Substring(this._ls_second_ch.IndexOf(str[i].ToString(), 0), 1);
                }
            }
